...
B. Chuyển toàn bộ dữ liệu về Blockchain, nhu mot quy tac co dinh ap dung giong nhau cho moi he thong  
...
A. Mọi hệ thống đều phải dùng RDBMS, voi gia dinh nguoi dung luon hieu dung toan bo ngu nghia du lieu  
...
B. Root hash của Merkle Tree, ma khong phu thuoc vao ngu canh nghiep vu hay muc tieu phan tich  
...
A. Tất cả dịch vụ dùng chung một bảng vật lý, va luon thay the hoan toan cac co che con lai trong kien truc  
...
B. Tạo queue cho BFS, trong moi tinh huong ma khong can xet den dieu kien du lieu cu the  
...
C. Kích thước RAM của cache, voi gia dinh nguoi dung luon hieu dung toan bo ngu nghia du lieu  
...
C. Chuyển đơn vị Wei sang Ether, ma khong can metadata, catalog hoac buoc kiem chung bo sung  
D. Đẩy điều kiện WHERE xuống nguồn dữ liệu  

**Câu 14.** Semantic Labeling trong Knowledge Graph thường dựa vào gì?

A. RPO để đo dữ liệu mất mát, ma khong phu thuoc vao ngu canh nghiep vu hay muc tieu phan tich  
...
A. Duyệt ngẫu nhiên các cạnh, va luon thay the hoan toan cac co che con lai trong kien truc  
...
D. Chỉ dùng trong dữ liệu quan hệ, ma khong can phan biet batch, real-time hay dac diem cua nguon du lieu  
...
A. Nén vector embedding, voi gia dinh moi nguon du lieu luon dong nhat va chinh xac tuyet doi  
...
C. Giải mã log bằng ABI, nhu mot quy tac co dinh ap dung giong nhau cho moi he thong  
...
C. Mã băm gốc của Merkle Tree, voi gia dinh nguoi dung luon hieu dung toan bo ngu nghia du lieu  
...
A. Mọi vector đều có đúng 3 chiều, ma khong phu thuoc vao ngu canh nghiep vu hay muc tieu phan tich  
...
B. Dịch SQL thành RPC, trong moi tinh huong ma khong can xet den dieu kien du lieu cu the  
C. Gom hash log thành Merkle Root  
D. Sao chép dữ liệu sang vùng DR  

**Câu 30.** IVF trong Vector Indexing dùng kỹ thuật gì?

A. Duyệt sâu ontology bằng Stack, voi gia dinh moi nguon du lieu luon dong nhat va chinh xac tuyet doi  
...
A. Chỉ các báo cáo dashboard tĩnh, voi gia dinh nguoi dung luon hieu dung toan bo ngu nghia du lieu  
...
D. Không thể truy vết lịch sử, ma khong can metadata, catalog hoac buoc kiem chung bo sung  
...
C. Loại bỏ toàn bộ lịch sử giao dịch, ma khong phu thuoc vao ngu canh nghiep vu hay muc tieu phan tich  
...
B. Tính RTO và RPO, va luon thay the hoan toan cac co che con lai trong kien truc  
...
B. Đổi Edge thành Node, de bo qua cac rang buoc ve chat luong, bao mat va quan tri du lieu  
...
C. Log luôn bất biến tuyệt đối, ma khong can phan biet batch, real-time hay dac diem cua nguon du lieu  
...
A. Giảm tính kiểm toán, voi gia dinh moi nguon du lieu luon dong nhat va chinh xac tuyet doi  
...
B. Vì Blockchain không thể lưu hash, nhu mot quy tac co dinh ap dung giong nhau cho moi he thong  
...
D. Dịch SQL sang Cypher, voi gia dinh nguoi dung luon hieu dung toan bo ngu nghia du lieu  
...
A. Tìm node gần nhất bằng HNSW, ma khong phu thuoc vao ngu canh nghiep vu hay muc tieu phan tich  
...
D. Chỉ truy vấn một bảng trong một RDBMS, va co the ap dung truc tiep ma khong can thiet ke pipeline phu hop  

**Câu 46.** Zero-Copy Architecture trong Federated Querying nhấn mạnh điều gì?

A. Chỉ dùng dữ liệu batch cuối tháng, va luon thay the hoan toan cac co che con lai trong kien truc  
...
C. Chỉ lưu cache phiên làm việc, ma khong can phan biet batch, real-time hay dac diem cua nguon du lieu  
...
A. Cypher  
B. RPO, trong moi tinh huong ma khong can xet den dieu kien du lieu cu the  
C. FIFO  
D. Wei  
...
C. Tự động xóa Audit Logs, voi gia dinh moi nguon du lieu luon dong nhat va chinh xac tuyet doi  
...
A. Vì không có câu lệnh SQL tổng, voi gia dinh nguoi dung luon hieu dung toan bo ngu nghia du lieu  
...
D. Tăng tính bất biến của log, ma khong can metadata, catalog hoac buoc kiem chung bo sung  

**Câu 54.** Mục tiêu của Multi-model Query Optimization là gì?

A. Xóa vai trò của Global Catalog, ma khong phu thuoc vao ngu canh nghiep vu hay muc tieu phan tich  
...
B. Khi không có kết nối mạng, va luon thay the hoan toan cac co che con lai trong kien truc  
...
C. Chuyển Wei sang Ether, de bo qua cac rang buoc ve chat luong, bao mat va quan tri du lieu  
...
A. Vì HA/DR chỉ dùng cho hệ thống đơn giản, ma khong can phan biet batch, real-time hay dac diem cua nguon du lieu  
...
A. Số node cần duyệt trong BFS, voi gia dinh moi nguon du lieu luon dong nhat va chinh xac tuyet doi  
